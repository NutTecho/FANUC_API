...
namespace TPPAPI.Controllers
{
    ...
    public class FanucInfoController : ControllerBase
    {
        ...
        [HttpGet("find-dll-path")]
        public IActionResult FindDllPath()
        {
            try
            {
                // ดึงรายการ Module ทั้งหมดที่ Process w3wp.exe (IIS) โหลดขึ้นแรมมาในขณะนั้น
                var currentProcess = System.Diagnostics.Process.GetCurrentProcess();
                string targetDllName = "fwlib64.dll"; // ค้นหาด้วยตัวพิมพ์เล็กเพื่อความยืดหยุ่น
                string actualPath = "ยังไม่ได้ถูกโหลดขึ้นหน่วยความจำ (หรือหาไม่เจอ)";

                foreach (System.Diagnostics.ProcessModule module in currentProcess.Modules)
                {
                    if (module.ModuleName != null && module.ModuleName.ToLower() == targetDllName)
                    {
                        actualPath = module.FileName; // พาธเต็มบนดิสก์ เช่น C:\...\FWlib64.dll
                        break;
                    }
                }
                ...
                return Ok(new
                {
                    ...
                    target_dll = targetDllName,
                    resolved_absolute_path = actualPath
                });
            }
            catch (Exception ex)
            {
